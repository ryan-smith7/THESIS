...
const uint16_t AS7343_WL[AS7343_NUM_CH] = {
...
{
    /*
     * WROVER has SPIRAM — place the large output buffer there to relieve
     * pressure on the 96 KB internal DRAM shared with BT + MQTT stacks.
     * ESP32-POE has no SPIRAM; the buffer stays in regular BSS.
     */
#if defined(CONFIG_ESP_SPIRAM)
    static char json_output[JSON_BUFFER_SIZE] __attribute__((section(".ext_ram.bss")));
#else
