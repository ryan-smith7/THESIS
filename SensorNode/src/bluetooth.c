...
/**
 * @brief Read callback for the custom characteristic to return packed sensor data (REDUNDANT FUNCTION THOUGH FILLS PLACEHOLDER).
 *
...
static void connected(struct bt_conn *conn, uint8_t err) {
    if (err) {
        LOG_ERR("[SENSOR] Failed to connect (err %u)", err);
        return;
    }

    current_conn = bt_conn_ref(conn);
    LOG_INF("[SENSOR] Connected");
    ble_tick = 1;
...
static void disconnected(struct bt_conn *conn, uint8_t reason) {
    ...
    LOG_ERR("[SENSOR] Disconnected (reason 0x%02x)", reason);
...
static void start_advertising_with_retry(bool after_disconnect) {
    ...
    while (1) {
        int err = bt_le_adv_start(&adv_params, ad, ARRAY_SIZE(ad),
                                   sd, ARRAY_SIZE(sd));
        if (err == 0) {
            LOG_INF("[SENSOR] Advertising started");
            return;
        }
        LOG_ERR("[SENSOR] Advertising failed (%d) — retrying in 1s", err);
        k_sleep(K_SECONDS(1));
    }
}
...
extern int init_bluetooth(void) {
    int err;
    LOG_INF("[SENSOR] Initializing Bluetooth...\n");
    err = bt_enable(NULL);
    if (err) {
        LOG_ERR("[SENSOR] Bluetooth init failed (err %d)\n", err);
        return err;
    }
...
extern int start_advertising(void) {
    int err;
    LOG_INF("[SENSOR] Starting advertising...");
    err = bt_le_adv_start(&adv_params, ad, ARRAY_SIZE(ad), sd, ARRAY_SIZE(sd));
    if (err) {
        LOG_ERR("[SENSOR] Advertising failed to start (err %d)", err);
        return err;
    }
...
extern int stop_advertising(void) {
    ...
    int err;
    LOG_INF("[SENSOR] Stopping advertising...");

    err = bt_le_adv_stop();
    if (err) {
        LOG_ERR("[SENSOR] Failed to stop advertising (err %d)", err);
        return err;
    }

    LOG_INF("[SENSOR] Advertising stopped");
    return 0;
}
...
extern int stop_advertising_and_disconnect() {
    ...
    if (err) {
        LOG_ERR("[SENSOR] Failed to stop advertising");
        return err;
    }
    ...
    if (current_conn) {
        ...
        if (err) {
            LOG_ERR("[SENSOR] Disconnection failed (err %d)", err);
            return err;
        } else {
            LOG_INF("[SENSOR] Disconnection initiated");
        }
    }
...
void sensornode_thread(void) {
    ...
    if (err) {
        LOG_ERR("[SENSOR] Bluetooth init failed (%d)", err);
        return;
    }
