/*
 * sntp_sync.c — Combined BLE + WiFi Gateway (ESP32 WROVER)
 *
 * Periodically queries an NTP server and calls time_sync_writer_set_utc()
 * directly. The time_sync_writer then distributes UTC to all connected
 * sensor nodes via GATT WRITE inside process_data_thread().
 *
 * On the combined gateway there is no UART bridge and no 0xCC frame —
