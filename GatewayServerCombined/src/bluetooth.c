/*
 * bluetooth.c — Combined BLE + WiFi Gateway (ESP32 WROVER)
 *
 ...
 *
 * On the combined WROVER gateway there is no UART bridge.
 * process_data_thread decodes each packet, encodes JSON, and publishes
 * directly via azure_mqtt_publish().
 *
...
/* Assembled sound UART payload reused as intermediate binary buffer */
...
static struct sound_reassembly sound_rx[MAX_CONN] __attribute__((section(".ext_ram.bss")));
 ...
 *
 * Layout matches uart_bridge.c decode_sensor() exactly.
 * Kept here so bluetooth.c is self-contained; uart_bridge.c is gone.
 * ════════════════════════════════════════════════════════ */
...
/* Stack must be large enough to run JSON encode without PSRAM stack spill.
 * 6 KB is sufficient; sensor JSON ~1.5 KB, sound JSON ~4 KB. */
...
/* ── Large working buffers — file scope so __noinit routes them to dram1 ── */
/* CONFIG_ESP32_REGION_1_NOINIT=y sends .noinit sections to dram1_0_seg.    */
/* All buffers are explicitly memset before first use in process_data_thread. */
...
{
    /* Zero all __noinit buffers before first use */
    ...
    while (1) {
        ...
        if (k_msgq_get(&sensor_msgq, &spkt, K_MSEC(5)) == 0) {
            ...
            /* In azure_mqtt_thread, change heartbeat to 686 bytes */
           
            // static char big_test[DUMMY];
            // memset(big_test, 'x', sizeof(big_test) - 1);
            // big_test[sizeof(big_test) - 1] = '\0';

            if (ret > 0 && ret < (int)sizeof(s_sensor_json)) {
                int pub = azure_mqtt_publish(s_sensor_json);
                // int pub = azure_mqtt_publish(big_test);
                ...
                if (pub < 0){
                    ...
                    // LOG_INF("Sensor published %d bytes)", DUMMY);
                }
            } else {
